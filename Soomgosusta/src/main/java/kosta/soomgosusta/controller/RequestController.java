...
public class RequestController {
	...
	public void detailRequest(@RequestParam("p_Seq") String p_Seq, @RequestParam("p_S_Word") String p_S_Word,
		...
		List<QuestionVO> listQuestion = service.listQuestion(listQ);
		List<AnswerVO> listAnswer = service.listAnswer(listA);

		HashMap<String, String> requestMap = getRequest(listQuestion, listAnswer, date, time, gen, sido, gugun, phoneNum);
		requestMap.put("p_Seq", p_Seq);
		requestMap.put("m_Id", "seungwon");
		
		int size = service.listmyRequest(requestMap);
		
		if(size == 0){
			service.insertRequest(requestMap, Integer.parseInt(p_Seq));
		}
	...
	public HashMap<String, String> getRequest(List<QuestionVO> question, List<AnswerVO> answer,
			String date, List<String> time, String gen, List<String> sido, List<String> gugun, String phoneNum){
